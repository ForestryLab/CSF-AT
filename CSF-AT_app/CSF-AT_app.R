...
calc_smartness <- function(df, pesi_df) {
  ...
  results <- lapply(seq_len(nrow(df)), function(i) {
    ...
    csf_sd  <- sd * pagg_sd
    csf_total <- csf_mit + csf_adp + csf_sd
    data.frame(
               ...
               column(6,
                      tags$div(class = "info-card top-card",
                               ...
                               tags$ul(
                                 ...
                                 tags$li(tags$b("Socio-economic dimension"), " – sustaining production and social benefits.")
                               )
                      )
               ),
               column(6,
                      ...
                      column(6,
                             tags$div(class = "info-card bottom-card",
                                      ...
                                      tags$ol(
                                        ...
                                        tags$li(tags$b("Aggregation"), " – using the Analytic Hierarchy Process (AHP) to produce ICSF scores for Mitigation, Adaptation, and Socio-economic dimensions.")
                                      )
                             )
                      ),
                      column(6,
                             tags$div(class = "info-card bottom-card",
                                      ...
                                      tags$ol(
                                        ...
                                        tags$li(tags$b("Visualize results"), " – In the Graphic Comparison panel, stand-level values are shown using an interactive ternary plot, where each point represents the balance among Mitigation, Adaptation, and Socio-economic dimension."),
...
server <- function(input, output, session) {
  ...
  output$ternary_plot <- renderPlotly({
    ...
    hover_txt <- ~paste(
      ...
      "<br>Year:", Year,
      "<br>N plots:", Nplots,
      "<br>Mitigation:", mit_raw,
      "<br>Adaptation:", adp_raw,
      "<br>Social Dimension:", sd_raw,
      "<br>CSF:", csf
    )
